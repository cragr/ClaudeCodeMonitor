...
/// Placeholder for SessionDetailView - will be implemented in Task 7
        ...
        VStack(spacing: Spacing.xl) {
            ...
            if !session.tokensByType.isEmpty {
                VStack(alignment: .leading, spacing: Spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: Spacing.xs) {
                        ForEach(Array(session.tokensByType.keys.sorted(by: { $0.rawValue < $1.rawValue })), id: \.self) { tokenType in
                            if let count = session.tokensByType[tokenType] {
                                TerminalDetailRow(label: tokenType.displayName, value: formatTokenCount(count))
                            }
                        }
                    }
                    .padding(Spacing.lg)
            ...
            if !session.tokensByModel.isEmpty {
                VStack(alignment: .leading, spacing: Spacing.md) {
                    ...
                    VStack(alignment: .leading, spacing: Spacing.xs) {
                        ForEach(Array(session.tokensByModel.keys.sorted()), id: \.self) { model in
                            if let count = session.tokensByModel[model] {
                                TerminalDetailRow(label: model, value: formatTokenCount(count))
                            }
                        }
                    }
                    .padding(Spacing.lg)
